...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    for data_type in INVERTER_DATA:
        ...
        sensors.append(
            SofarNetDataSensor(name, uid, entity_id, sensor_name, updater, data_type)
        )
...
INVERTER_ALARM_CODES = {
    ...
    "14": [
        "HwBoostOCP",
        "The input current is too high, and has happen hardware protection",
    ],
}
...
class SofarNetDataUpdater:
    ...
    def _update_data(self):
        ...
        if elevation <= self._min_elevation:
            _LOGGER.debug(
                f"Current sun elevation: {elevation} > min {self._min_elevation}?"
            )
            INVERTER_DATA[POWER_NOW_TAG][INVERTER_DATA_VALUE_AT] = 0.0
            if sun_rising:
                _LOGGER.debug(
                    f"Sun rising: {sun_rising} elevation: {elevation} > min {self._min_elevation}?"
                )
        ...
        if elevation > self._min_elevation:
            try:
                ...
                for data_type in INVERTER_DATA:
                    INVERTER_DATA[data_type][INVERTER_DATA_VALUE_AT] = GetValueForTag(
                        data_from_inverter.text, data_type
                    )
                    if data_type == TODAY_ENERGY_TAG:
                        INVERTER_DATA[data_type][INVERTER_DATA_VALUE_AT] = FixSofarData(
                            INVERTER_DATA[data_type][INVERTER_DATA_VALUE_AT]
                        )
                    # if data_type == INVERTER_ALARM_TAG:                             # for testing
                    #    INVERTER_DATA[data_type][INVERTER_DATA_VALUE_AT] = 'F12F14' # for testing
                    if (
                        data_type == INVERTER_ALARM_TAG
                        and len(INVERTER_DATA[data_type][INVERTER_DATA_VALUE_AT]) > 0
                    ):
                        INVERTER_DATA[data_type][INVERTER_DATA_VALUE_AT] = (
                            InverterAlarmDecode(
                                INVERTER_DATA[data_type][INVERTER_DATA_VALUE_AT]
